	...
	for (;;) {
		...
		if (fifo8_status(&keyfifo) + fifo8_status(&mousefifo) + fifo8_status(&timerfifo) == 0) {
			...
			if (fifo8_status(&keyfifo) != 0) {
				...
				io_sti();
                char s2;
				sprintf(s2, "%d count", a);
					boxfill8(buf_back, binfo->scrnx, COL8_008484, 0, 64, 79, 320);
				putfonts8_asc(buf_back, binfo->scrnx, 0, 64, COL8_FFFFFF, s2);
